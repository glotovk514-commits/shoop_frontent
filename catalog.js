const API_URL = 'https://shoop-bakend.onrender.com/api/products';
const ORDERS_URL = 'https://shoop-bakend.onrender.com/api/orders';
...
function openProductDetails(id) {
    ...
    detailCategory.innerText = getCategoryName(product.category);
    detailDescription.innerText = product.description || 'Описание отсутствует';
    
    detailCurrentPrice.innerText = `${product.price} руб.`;
    detailOriginalPrice.innerText = product.originalPrice ? `${product.originalPrice} руб.` : '';

    detailSpecsList.innerHTML = product.specifications && product.specifications.length > 0 
        ? product.specifications.map(s => `<li>• ${s}</li>`).join('') 
        : '<li>Уточняйте характеристики у менеджера</li>';
...
        card.className = `product-card loaded ${product.featured ? 'featured' : ''}`; 
...
                    <img src="${item.image}" height="150" width="150" style="object-fit: contain;">
                    <h4>${item.name}</h4>
